...
영어·일본어·중국어 등 단어와 표현을 한국어로 조회해 Anki용 TSV 파일에 자동 저장하는 앱입니다.
...
`AnkiWordCollector_Android.apk`를 휴대폰으로 옮겨 설치하세요. 앱에서 단어 또는 표현을 조회하면 자동 저장됩니다. **TSV 내보내기**를 누른 뒤 저장할 위치를 선택하면 `anki_words.tsv`를 만들 수 있으며, 그 파일을 Anki에 가져오면 됩니다.

화면 위쪽에서 **기사시험 개념 노트**를 선택하면, 예를 들어 `정규화`와 `데이터 중복을 줄이는 설계 과정`처럼 개념과 본인 정리를 저장할 수 있습니다. **개념 TSV 내보내기**를 누르면 `exam_notes.tsv`로 따로 저장됩니다.
